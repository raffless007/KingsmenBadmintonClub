(function () {
  ...
  const enhancement = { installed: false, originalRender: null, supabase: null, channel: null, pollTimer: null, alertPollTimer: null, flushing: false, adminRoles: [], adminAnnouncements: [], lastAdminTab: null };
  ...
  function renderAdminRoleLogin() {
    ...
    select.innerHTML = `<option value="">${players.length ? "Choose player" : "No players loaded"}</option>${players.map((player) => `<option value="${player.id}">${esc(player.name)}</option>`).join("")}`;
